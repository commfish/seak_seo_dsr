...
meters.surv<-DAT %>% group_by(Sample.Label) %>% 
  summarize(effort.tot = mean(Effort),
            fish.tot = n())
tot.surv<-sum(meters.surv$effort.tot)
fish.tot<-sum(meters.surv$fish.tot)
er<-fish.tot/tot.surv
...
## Depending on what covariates are included, run the appropriate model
		...
		if (TRUNC == "Y") {
			if (FORM == "~Depth") {
			  withTimeout((try(invisible(capture.output(M <- ds(R2, key = KEY, adjustment = ADJ, transect="line", formula=~Depth,
			                                                   convert_units = CU, truncation=c(right="5%"), quiet=TRUE),type="message")),
			                  silent=TRUE)),timeout=600)
			} else if (FORM == "~Stage") {
			  withTimeout((try(invisible(capture.output(M <- ds(R2, key = KEY, adjustment = ADJ, transect="line", formula=~Stage,
			                                                   convert_units = CU, truncation=c(right="5%"), quiet=TRUE),
			                                           type="message")),silent=TRUE)),timeout=600)
			} else if (FORM == "~Depth + Stage") {
			  withTimeout((try(invisible(capture.output(M <- ds(R2, key = KEY, adjustment = ADJ, transect="line", formula=~Depth + Stage,
			                                                   convert_units = CU, truncation=c(right="5%"), quiet=TRUE),
			                                           type="message")),silent=TRUE)),timeout=600)
			} else {
			  withTimeout((try(invisible(capture.output(M <- ds(R2, key = KEY, adjustment = ADJ, transect="line", formula=~1,
			                                                   convert_units = CU, truncation=c(right="5%"), quiet=TRUE),
			                                           type="message")),silent=TRUE)),timeout=600)
			}
		} else {
			if (FORM == "~Depth") {
			  withTimeout((try(invisible(capture.output(M <- ds(R2, key = KEY, adjustment = ADJ, transect="line", formula=~Depth,
			                                                   convert_units = CU, quiet=TRUE),
			                                           type="message")),silent=TRUE)),timeout=600)
			} else if (FORM == "~Stage") {
			  withTimeout((try(invisible(capture_output(M <- ds(R2, key = KEY, adjustment = ADJ, transect="line", formula=~Stage,
			                                                   convert.units = CU, quiet=TRUE),
			                                           type="message")),silent=TRUE)),timeout=600)
			} else if (FORM == "~Depth + Stage") {
			  withTimeout((try(invisible(capture.output(M <- ds(R2, key = KEY, adjustment = ADJ, transect="line", formula=~Depth + Stage,
			                                                   convert_units = CU, quiet=TRUE),
...
MB<-Model.Boot
write.csv(MB, file = "Output/Density_CSEO_22_Trunc_Model_Avg_boot_1000.csv")
...
##or
par(mfrow = c(2,2),mar=c(4,4,4,1)+0.1,oma=c(1,1,1,1))
...
quantile(MB$Dhat, c(0.025,0.05,0.1,0.9,0.95, 0.975))
