...
species <-read.csv("SSEO_2020_Species.csv")
str(species)
...
DSR<-species[species$Family == "rockfish",]
view(DSR)
unique(DSR$Genus)

COL<-brewer.pal(length(unique(DSR$Genus)),name="Set3")
g<-ggplot(DSR, aes(Genus))
g+geom_bar(fill=COL)+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

s<-ggplot(species, aes(Family)) 
s+geom_bar(aes(fill=Genus))+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
