...
## original efort did it wrong and resampled distances within transects...
#########################################################################
library(tictoc)
...
MB<-Model.Boot
write.csv(MB, file = "Output/Model_Avg_boot_GIStran_1000.csv")
######################################################################################
## Examine bootstrap
######################################################################################
nrow(MB)
...
plot(density(MB$Dhat), xlab="Nhat", col="goldenrod2", main="Bootstrapped SSEO Yelloweye Density")
	polygon(density(MB$Nhat), col="goldenrod2")
	abline(v=mean(MB$Nhat), col="red", lty=1)
	abline(v=median(MB$Nhat), col="red", lty=2)
	abline(v=quantile(MB$Nhat, c(0.1,0.9)), col="darkorange")
