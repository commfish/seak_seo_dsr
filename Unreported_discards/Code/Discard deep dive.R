...
str(Survey)
  ...
  YEHA.fxn<-function(Area="In.Out",Deep=max(Survey$AvgDepth.fm), Shallow=0, nboot=500){
      ...
      for (s in Subs){  #s<-Subs[1]
        ...
        if (nrow(Dat)>0){
          ...
          for (st in Stations){    #st<-Stations[1]   length(Stations)  st<-1
            ...
            Ci[i]<-C
            CPUEi[i]<-CPUE
            Hal.O32[i]<-mean(Stat.Dat$O32.Pacific.halibut.weight)
            Hal.all[i]<-mean(Stat.Dat$O32.Pacific.halibut.weight+Stat.Dat$U32.Pacific.halibut.weight)
            K<-C*Dat$mean.YE.kg[1]
            WCPUE.32 <-K/mean(Stat.Dat$O32.Pacific.halibut.weight)
            WCPUE.all<-K/mean(Stat.Dat$O32.Pacific.halibut.weight+Stat.Dat$U32.Pacific.halibut.weight)
            WCPUEi.32[i]<-WCPUE.32
          ...
          #get rid of NA's where no halibut were caught!!! 
          forwavg32<-as.data.frame(cbind(WCPUEi.32,Hal.032))
          forwavgall<-as.data.frame(cbind(WCPUEi.all,Hal.all))
          
          WCPUEi.32<-WCPUEi.32[!is.na(WCPUEi.32)]
          WCPUEi.all<-WCPUEi.all[!is.na(WCPUEi.all)]
          Hal.032<-Hal.O32[!is.na(Hal.O32)]
          Hal.all<-Hal.O32[!is.na(Hal.all)]
          forwavg32<-forwavg32[complete.cases(forwavg32),]
          forwavgall<-forwavgall[complete.cases(forwavgall),]
          
          n32<-seq(1,nrow(forwavg32),by=1)
          nall<-seq(1,nrow(forwavgall),by=1)
          #bootstrap WCPUE
          Out<-data.frame()
          CPUE.out<-data.frame()
          for (ii in 1:nboot){ #nboot<-1000  ii<-1
            ...
            for (r in 1:length(n32)){  #r<-2
              rn<-sample(n32, size=1,replace=TRUE)
              R1<-forwavg32[rn,]
              if (r == 1) {
                Resamp4<-R1
              } else {
                Resamp4<-as.data.frame(rbind(Resamp4,R1))
              }  #nrow(R2)
            }
            Out[ii,"wWCPUE.32"]<-weighted.mean(Resamp4$WCPUEi.32,w= Resamp4$Hal.032,na.rm=T)
            #Out[ii,"var.wWCPUE.32"]<-var(Resamp4, na.rm=T)
            for (r in 1:length(nall)){  #r<-2
              rn<-sample(nall, size=1,replace=TRUE)
              R1<-forwavgall[rn,]
              if (r == 1) {
                Resamp5<-R1
              } else {
                Resamp5<-as.data.frame(rbind(Resamp5,R1))
              }  #nrow(R2)
            }
            Out[ii,"wWCPUE.all"]<-weighted.mean(Resamp5$WCPUEi.all,w= Resamp5$Hal.all,na.rm=T)
            #bootstrap bycatch and discards 12-20-21: exact same results if you pu
            #Out[ii,"YE.bycatch.kg"]<-sum(Harv$round_lbs_hali)*0.45359237*Out[ii,"WCPUE"]
            #Out[ii,"raw.YE.discards.kg"]<-Out[ii,"YE.bycatch.kg"]-Land.YEby$YE_fr_hal_fish.kgs #max(1,Out[ii,"YE.bycatch.kg"]-Land.YEby$YE_fr_hal_fish.kgs, na.rm=T)
            #min.D.boot<-min(1,rnorm(1,mean=min.Discard ,sd=sqrt(min.D.var))*Land.YEby$YE_fr_hal_fish.kgs)
          ...
          IPHC.wcpue[j,"ye.mean.catch"]<-mean(Ci)
          IPHC.wcpue[j,"ye.mean.kg"]<-Dat$mean.YE.kg[1]
          IPHC.wcpue[j,"yelloweye.meancpue.nph"]<-mean(CPUEi)
          IPHC.wcpue[j,"halibut.O32.meankg"]<-mean(Hal.O32)
          IPHC.wcpue[j,"halibut.all.meankg"]<-mean(Hal.all)
          ...
          IPHC.wcpue[j,"wWCPUE32.mean"]<-weighted.mean(forwavg32$WCPUEi.32, w=forwavg32$Hal.032)
          IPHC.wcpue[j,"wWCPUE32.bootmean"]<-unname(quantile(Out$wWCPUE.32,c(0.5)))  #mean WCPUE from Tribuzio
          IPHC.wcpue[j,"wWCPUE32.lo95ci"]<-unname(quantile(Out$wWCPUE.32,c(0.025)))
          IPHC.wcpue[j,"wWCPUE32.hi95ci"]<-unname(quantile(Out$wWCPUE.32,c(0.975)))
          IPHC.wcpue[j,"wWCPUE32.var"]<-var(Out$wWCPUE.32)
          IPHC.wcpue[j,"wWCPUE32.cv"]<-sd(Out$wWCPUE.32)/mean(Out$wWCPUE.32)
          
          IPHC.wcpue[j,"wWCPUEall.mean"]<-weighted.mean(forwavgall$WCPUEi.all, w=forwavgall$Hal.all)
          IPHC.wcpue[j,"wWCPUEall.bootmean"]<-unname(quantile(Out$wWCPUE.all,c(0.5)))  #mean WCPUE from Tribuzio
          IPHC.wcpue[j,"wWCPUEall.lo95ci"]<-unname(quantile(Out$wWCPUE.all,c(0.025)))
          IPHC.wcpue[j,"wWCPUEall.hi95ci"]<-unname(quantile(Out$wWCPUE.all,c(0.975)))
          IPHC.wcpue[j,"wWCPUEall.var"]<-var(Out$wWCPUE.all)
          IPHC.wcpue[j,"wWCPUEall.cv"]<-sd(Out$wWCPUE.all)/mean(Out$wWCPUE.all)
 ...
 write.csv(SE.wcpue,"Output/SE.wcpue.ests_6.24.22.csv")
 
 #CPUE for RPN's and abundance index
 
 SE.cpue<-YEHA.fxn(Area="SEdist",Deep=250, Shallow=0, nboot=1000)
 head(SE.cpue)
