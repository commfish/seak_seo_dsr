...
theme_set(theme_bw(base_size=18,base_family="Times")
...
## halibut_catch_data
## FLAG!!! 1996 data seems off - it is a lot higher than the IPHC data - need to circle back and
## figure out why
...
## specifically the TSD-026. In 2026 this report was titles - Commercial landings
## by IPHC Statistical Area. These data are derived from logbook data and fish 
## tickets. This report in particular lags by about a year. For example, the 2024 
## logs will still  be collected well into 2025 and IPHC will generate an 
...
hal_fishery$Year <- as.numeric(as.character(hal_fishery$Year))

hal_summary <- hal_fishery %>%
  group_by(Year, Mgt.Area, source) %>%
  summarise(
    ha.mt = sum(ha.mt, na.rm = TRUE),.groups = "drop")

ggplot(hal_summary, aes(Year, ha.mt, col = source)) +
  geom_line() +
  facet_wrap(~Mgt.Area)+
  scale_x_continuous(
    breaks = seq(min(hal_fishery$Year),
                 max(hal_fishery$Year),
                 by = 2)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

## FLAG!!! The harvest in NSEI seems remarkably high - is this true? Or is there an error in the data?

# ## I am trying to sort out what is bycatch and what are directed trips
# ## There are dual target trips in sablefish, DSR, and p.cod fisheries.
...
Halibut.harv.1975 <- hal_fishery %>%
  #FLAG!!! What filters are best here? I filtered for B permits and longline gear 
  #adding filter for B permits and longline gear but  will filter out years 1976 and 1975
  #because fishery codes weren't used yet
  #Longline is the dominate gear type but dinglebar troll, mechanical jig, pots, 
  #and power trolll are also seen in the data.
  #I made an entire powerpoint about this incase anyone wants to go down the rabbit hole about it...LSC 6/22/26
...
HA.newreq <- HA.newreq %>% 
  ...
  mutate(Net_lbs=Net.wt..lb./1000) %>% #converting to thousands of lbs to match Randy's data request
  select(Year,IPHC.Regulatory.Area,IPHC.Statistical.Area = IPHC..Statistical.Area,Net_lbs)

HA.newreq <- left_join(HA.newreq,IOs,by="IPHC.Statistical.Area")
...
#halibut harvest in SEO 2C from IPHC
...
data_check_plot <- Hal.SPM %>%
  ...
  scale_x_continuous(breaks = seq(min(Hal.SPM$Year), max(Hal.SPM$Year), by = 1)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  ylim(0, 3500) +
  labs(
...
## FLAG!!! Large mismatch between ADFG FT data and IPHC data in 1996 and I remeber chasing this
## down in 2025 and was not able to figure it out.
...
#Plotting to make sure the IPHC reconstructed data matches the IPHC web data
HA.IPHCweb %>% 
  select(Year = Year,
         a2C.mt=X2C.mt,
...
harv_summary <- Halibut.harv.1975 %>%
  ...
  summarise(EYKT = sum(HA.mt[Mgt.Area == "EYKT"], na.rm = TRUE),
            SEO2C = sum(HA.mt[Mgt.Area %in% c("NSEO", "CSEO", "SSEO")], na.rm = TRUE),
            SEI2C = sum(HA.mt[Mgt.Area %in% c("NSEI", "SSEI")], na.rm = TRUE),.groups = "drop") %>%
