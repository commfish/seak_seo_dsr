...
#priortry[priortry<0.]
Ms<-c(0.03) #c(0.015,0.02,0.025,0.03) #,0.04)
Zs<-c(0.055)#c(0.04,0.05,0.06)
AaMs<-c(17)
Years<-500
A<-120
  ...
  for (z in Zs){ #z<-Zs[1]
    for (as in AaMs){  #as<-AaM[1]
      ...
      parameters=c("r","r.check",
        ...
        tmp <- uniroot( gammafind, lower=0.1, upper=10 )
...
#++++++++++++++++****************************+++++++++++++++++
#+Quick check... 
R.out<-read.csv("Model Output/r_prior_dev_leslie_100k/R_prior_search.csv")
...
png(paste("Figures//r_prior_fits.png", sep=""),
    width=7,height=4,#width=9.5,height=8.5,
    units="in",res=1200)
