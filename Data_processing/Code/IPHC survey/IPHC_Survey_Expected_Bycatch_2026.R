...
## FLAG!! the note above is from Phil in 2023 
...
### SET UP BIODATA
################################################################################
## Notes from Phil: Get best data for average weights and add columns to survey 
## data. Because many years have inadequate sample sizes we will use the most 
## recent year's prior to a given year for the average. 
  ...
  for (i in Years){    #i<-Years[1]
    for (j in GFMA){   #j<-GFMA[1]
      if (i < 2008){
        ...
        m<-0
        while (Nw < 300 & i+m <= max(Port.rand$Year)) {  #go forward if you failed to find enough weights in the first couple of years 
          m<-m+1                                                        
...
ggplot(data = Survey %>% 
  ...
  facet_wrap(~SEdist) +
  xlab("\nPacific Halibut (> 32 cm) Catch (lbs)") +
  ylab("Density") +
  ylim(0,0.0033)#+
