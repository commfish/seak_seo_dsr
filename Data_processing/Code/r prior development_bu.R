...
nrow(Fem)
flage<-subset(Port,!is.na(Age) & !is.na(Length.Millimeters))
fwage<-subset(Port,!is.na(Weight.Kilograms) & !is.na(Age))
...
R.out<-data.frame()
niter<-100000
dir.create(paste("Model Output/",Mod.title,"_",niter/1000,"k_r_search",sep=""))
  ...
  for (z in Zs){ #z<-Zs[1]
    for (as in AaMs){  #as<-AaM[1]
      ...
      parameters=c("r","r.check",
        ...
        tmp <- uniroot( gammafind, lower=1, upper=10 )
...
oldr<-read.csv("Model Output/r_prior_dev_leslie_100k/r_search.csv")
R.out<-cbind(oldr,R.out)
R.out<-unique(Rss)
