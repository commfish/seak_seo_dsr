...
tau_b1 <- 0 #extra variance in early part of time series (0 means there is none); 
...
data <- simulate2(sim = sim, 
                      bio_srv_frq = 4, # how frequent are biomass surveys? 4 = every four years. 
...
#-------------------------------------------------------------------------------
# Run the model!! 
#500000 took 2.3 hours. not converged; shit divergences pella_strat_missdat2_peOpt_indOpt2.stan
#500K took 4.4 hours when I fixed sigma and still sucks! 
# 50K with control at 0.99/0.001: 1.8 hours.  not mixed at all... all divergent 
# 500K base settings, 0.5 hours unconverged.
# 2M base settings, 2/3.6/16 hour on 3 chains.  Not converged...
#                   estimating without process error definitely causing issues with P estimates
# 2M base setting, better P inits and process error ON:  7.6 hours for longest chain
# 3M base.  Closer but not all the way there. 2 chains 4.7 hours each, 3rd chain 35 hours! (chain 2)
...
model = 'PT_NC_margQ_xV_D.stan' #'PT_C_margQ_xV.stan' # PT_NC_xV.stan' # PT_NC_margQ_xV.stan'
...
tstart <- Sys.time()
fit <- stan(file = paste0("code/",model), data = stan_dat, 
...
saveRDS(save_inits,"inits.rds")

tuned_inits<-readRDS("inits.rds")
# check if you have the right number of chains
str(inits)

tstart <- Sys.time()
fit2 <- stan(file = paste0("code/",model), data = stan_dat, 
            iter = iters, chains = chains, cores=chains,
...
plot_pe(fit = fit, sim_pe = pe, sim=TRUE, save=TRUE, mod_name=mod_name)
