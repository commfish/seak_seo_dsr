...
results_nope_B_I <- results_nope_I <- data.frame()
nsims <- 25
...
for (opts in harv_opts) { # opts <- harv_opts[1]
  ...
  for (fs in 1:length(fpres)){ # fs <- 1
    ...
    for (i in 1:nsims){ #i <- 1
      ...
      # run no PE models first
      tstart <- Sys.time()
      fit_nope_B_I <- stan(file = paste0("Production_models_STAN/Models/nope_estB_estI.stan"), 
      ...
      rhats <- check_convergence(fit_nope_B_I)
      
      add_iter <- stan_iters
      
      while (any(rhats > 1.1) && sum(fit_nope_B_I@sim$iter) < maxiter) {
      ...
      plot_biomass(fit=fit_nope_B_I,years=Nyear,strata=Narea,
                   save=FALSE, 
                   bio_dat = B_ests, bio_cv = B_cvs,
                   sim=TRUE, 
                   sim_bio = B, mod_name="Model fit_nope_B_I",
                   units = "NA", year1=1) -> bio_plot
      
      plot_index1(fit=fit_nope_B_I,years=Nyear,strata=Narea,
                  save=FALSE, 
                  cpue_dat = I_obs_plot, cpue_cv = I_cv_plot,
                  sim=TRUE, 
                  sim_cpue = sim_IAs, mod_name="Model fit_nope_B_I",
                  units = "NA",
                  year1 = 1,
                  cpue_name = "Fishery CPUE",
                  cpue_type = "fish/effort") -> index_plot
      
      #plot_pe1(fit = fit_nope_B_I, sim_pe = epsilon, sim=TRUE, save=FALSE, mod_name="Model fit_nope_B_I", year1 = 1, strata = Narea) -> pe_plot
      
      #ggarrange(bio_plot,index_plot,pe_plot)
      
      samples <- rstan::extract(fit_nope_B_I, permuted = TRUE)
      ...
      results_nope_B_I[data_iter,"model"] <- "fit_nope_B_I"
      results_nope_B_I[data_iter,"iter"] <- data_iter
      results_nope_B_I[data_iter,"harv_hist"] <- h_hist
      results_nope_B_I[data_iter,"Hmax"] <- Hmax
      
      results_nope_B_I[data_iter,"runtime"] <- runtime
      results_nope_B_I[data_iter,"iterations"] <- add_iter
      results_nope_B_I[data_iter,"param_not_conv"] <- sum(summary(fit_nope_B_I)$summary[,"Rhat"] > 1.1, na.rm=T)
      results_nope_B_I[data_iter,"div_trans"] <- get_num_divergent(fit_nope_B_I)
      results_nope_B_I[data_iter,"over_max_treedepth"] <- get_num_max_treedepth(fit_nope_B_I)
      ...
      for (s in 1:Narea){ #s<-1
        ...
        results_nope_B_I[data_iter,paste0("r_e_",s)] <- r_e[,s][2]
        results_nope_B_I[data_iter,paste0("K_e_",s)] <- K_e[,s][2]
        results_nope_B_I[data_iter,paste0("Bterm_e_",s)] <- Bterm[,s][2]
        results_nope_B_I[data_iter,paste0("q_e_",s)] <- q_e[,s][2]
        
        results_nope_B_I[data_iter,paste0("pe_e",s)] <- pe_e[,s][2] #***
        results_nope_B_I[data_iter,paste0("pe_e",s)] <- pe_e[,s][2] #***
        
        results_nope_B_I[data_iter,paste0("rbias_",s)] <- (r_e[,s][2] - sim_stats[data_iter,paste0("r_",s)]) / sim_stats[data_iter,paste0("r_",s)]
        results_nope_B_I[data_iter,paste0("r_wi_bnds_",s)] <- ifelse(sim_stats[data_iter,paste0("r_",s)] > r_e[,s][1] & 
                                                                           sim_stats[data_iter,paste0("r_",s)] < r_e[,s][3],
                                                                         "TRUE","FALSE")
        results_nope_B_I[data_iter,paste0("Kbias_",s)] <- (K_e[,s][2] - sim_stats[data_iter,paste0("K_",s)]) / sim_stats[data_iter,paste0("K_",s)]
        results_nope_B_I[data_iter,paste0("K_wi_bnds_",s)] <- ifelse(sim_stats[data_iter,paste0("K_",s)] > K_e[,s][1] & 
                                                                           sim_stats[data_iter,paste0("K_",s)] < K_e[,s][3],
                                                                         "TRUE","FALSE")
        results_nope_B_I[data_iter,paste0("pebias_",s)] <- NA
        results_nope_B_I[data_iter,paste0("pe_wi_bnds_",s)] <- NA
        
        results_nope_B_I[data_iter,paste0("abspebias_",s)] <- NA
        results_nope_B_I[data_iter,paste0("abspe_wi_bnds_",s)] <- NA
        
        results_nope_B_I[data_iter,paste0("SSbias_",s)] <- (Stock_status_e[,s][2] - sim_stats[data_iter,paste0("Depl_",s)]) / sim_stats[data_iter,paste0("Depl_",s)]
        ...
        results_nope_B_I[data_iter,paste0("MSYS_wi_bnds_",s)] <- ifelse(sim_stats[data_iter,paste0("MSY_t_",s)] > MSY_e[,s][1] & 
                                                                            ...
                                                                            "TRUE","FALSE")
        results_nope_B_I[data_iter,paste0("Bmsybias_",s)] <- (Bmsy_e[,s][2] - sim_stats[data_iter,paste0("Bmsy_t_",s)]) / sim_stats[data_iter,paste0("Bmsy_t_",s)]
        results_nope_B_I[data_iter,paste0("Bmsy_wi_bnds_",s)] <- ifelse(sim_stats[data_iter,paste0("Bmsy_t_",s)] > Bmsy_e[,s][1] & 
                                                                              sim_stats[data_iter,paste0("Bmsy_t_",s)] < Bmsy_e[,s][3],
                                                                            "TRUE","FALSE")
        results_nope_B_I[data_iter,paste0("Fmsybias_",s)] <- (Fmsy_e[,s][2] - sim_stats[data_iter,paste0("Fmsy_t_",s)]) / sim_stats[data_iter,paste0("Fmsy_t_",s)]
        results_nope_B_I[data_iter,paste0("Fmsy_wi_bnds_",s)] <- ifelse(sim_stats[data_iter,paste0("Fmsy_t_",s)] > Fmsy_e[,s][1] & 
      ...
      fit_nope_I <- stan(file = paste0("Production_models_STAN/Models/nope_estB_estI.stan"), 
                           ...
                           control = list(adapt_delta = adapt_delta, stepsize = stepsize, 
                                          max_treedepth = max_treedepth, stepsize_jitter = stepsize_jitter)) #stepsize_jitter default(0), values between 0 and 1
      #metric (string, one of "unit_e", "diag_e", "dense_e", defaults to "diag_e")
      rhats <- check_convergence(fit_nope_I)
      
      add_iter <- stan_iters
      
      while (any(rhats > 1.1) && sum(fit_nope_I@sim$iter) < maxiter) {
        print("Model not converged. Running additional chains...")
        add_iter <- add_iter + 5000
        init_ll_1r <- lapply(1:chains, function(id) init_create_1r(chain_id = id))
        #fit <- run_stan(iter = initial_iter + additional_iter, chains = initial_chains)
      ...
      plot_biomass(fit=fit_nope_I,years=Nyear,strata=Narea,
                   save=FALSE, 
                   bio_dat = B_ests, bio_cv = B_cvs,
                   sim=TRUE, 
                   sim_bio = B, mod_name="Model fit_nope_I",
                   units = "NA", year1=1) -> bio_plot
      
      plot_index1(fit=fit_nope_I,years=Nyear,strata=Narea,
                  save=FALSE, 
                  cpue_dat = I_obs_plot, cpue_cv = I_cv_plot,
                  sim=TRUE, 
                  sim_cpue = sim_IAs, mod_name="Model fit_nope_I",
                  units = "NA",
                  year1 = 1,
                  cpue_name = "Fishery CPUE",
                  cpue_type = "fish/effort") -> index_plot
      
      #plot_pe1(fit = fit_nope_I, sim_pe = epsilon, sim=TRUE, save=FALSE, mod_name="Model fit_nope_I", year1 = 1, strata = Narea) -> pe_plot
      
      #ggarrange(bio_plot,index_plot,pe_plot)
      
      samples <- rstan::extract(fit_nope_I, permuted = TRUE)
      MSY_e = Bmsy_e = Fmsy_e = Stock_status_e <-data.frame(matrix(NA, nrow = Narea, ncol = 3))
      r_e = K_e = Bterm = q_e = pe_e = pe_abs_e = data.frame(matrix(NA, nrow = Narea, ncol = 3)) #***
      
      results_nope_I[data_iter,"model"] <- "fit_nope_I"
      results_nope_I[data_iter,"iter"] <- data_iter
      results_nope_I[data_iter,"harv_hist"] <- h_hist
      results_nope_I[data_iter,"Hmax"] <- Hmax
      
      results_nope_I[data_iter,"runtime"] <- runtime
      results_nope_I[data_iter,"iterations"] <- add_iter
      results_nope_I[data_iter,"param_not_conv"] <- sum(summary(fit_nope_I)$summary[,"Rhat"] > 1.1, na.rm=T)
      results_nope_I[data_iter,"div_trans"] <- get_num_divergent(fit_nope_I)
      results_nope_I[data_iter,"over_max_treedepth"] <- get_num_max_treedepth(fit_nope_I)
      ...
      for (s in 1:Narea){ #s<-1
        ...
        #SS_obs[s] = tail(na.omit(B_ests[,s]), 1) / 
        results_nope_I[data_iter,paste0("r_e_",s)] <- r_e[,s][2]
        results_nope_I[data_iter,paste0("K_e_",s)] <- K_e[,s][2]
        results_nope_I[data_iter,paste0("Bterm_e_",s)] <- Bterm[,s][2]
        results_nope_I[data_iter,paste0("q_e_",s)] <- q_e[,s][2]
        
        results_nope_I[data_iter,paste0("pe_e",s)] <- pe_e[,s][2] #***
        results_nope_I[data_iter,paste0("pe_e",s)] <- pe_e[,s][2] #***
        
        results_nope_I[data_iter,paste0("rbias_",s)] <- (r_e[,s][2] - sim_stats[data_iter,paste0("r_",s)]) / sim_stats[data_iter,paste0("r_",s)]
        results_nope_I[data_iter,paste0("r_wi_bnds_",s)] <- ifelse(sim_stats[data_iter,paste0("r_",s)] > r_e[,s][1] & 
                                                                       sim_stats[data_iter,paste0("r_",s)] < r_e[,s][3],
                                                                     "TRUE","FALSE")
        results_nope_I[data_iter,paste0("Kbias_",s)] <- (K_e[,s][2] - sim_stats[data_iter,paste0("K_",s)]) / sim_stats[data_iter,paste0("K_",s)]
        results_nope_I[data_iter,paste0("K_wi_bnds_",s)] <- ifelse(sim_stats[data_iter,paste0("K_",s)] > K_e[,s][1] & 
                                                                       sim_stats[data_iter,paste0("K_",s)] < K_e[,s][3],
                                                                     "TRUE","FALSE")
        results_nope_I[data_iter,paste0("pebias_",s)] <- NA
        results_nope_I[data_iter,paste0("pe_wi_bnds_",s)] <- NA
        
        results_nope_I[data_iter,paste0("abspebias_",s)] <- NA
        results_nope_I[data_iter,paste0("abspe_wi_bnds_",s)] <- NA
        
        results_nope_I[data_iter,paste0("SSbias_",s)] <- (Stock_status_e[,s][2] - sim_stats[data_iter,paste0("Depl_",s)]) / sim_stats[data_iter,paste0("Depl_",s)]
        ...
        results_nope_I[data_iter,paste0("MSYS_wi_bnds_",s)] <- ifelse(sim_stats[data_iter,paste0("MSY_t_",s)] > MSY_e[,s][1] & 
                                                                          sim_stats[data_iter,paste0("MSY_t_",s)] < MSY_e[,s][3],
                                                                        "TRUE","FALSE")
        results_nope_I[data_iter,paste0("Bmsybias_",s)] <- (Bmsy_e[,s][2] - sim_stats[data_iter,paste0("Bmsy_t_",s)]) / sim_stats[data_iter,paste0("Bmsy_t_",s)]
        results_nope_I[data_iter,paste0("Bmsy_wi_bnds_",s)] <- ifelse(sim_stats[data_iter,paste0("Bmsy_t_",s)] > Bmsy_e[,s][1] & 
                                                                          sim_stats[data_iter,paste0("Bmsy_t_",s)] < Bmsy_e[,s][3],
                                                                        "TRUE","FALSE")
        results_nope_I[data_iter,paste0("Fmsybias_",s)] <- (Fmsy_e[,s][2] - sim_stats[data_iter,paste0("Fmsy_t_",s)]) / sim_stats[data_iter,paste0("Fmsy_t_",s)]
        results_nope_I[data_iter,paste0("Fmsy_wi_bnds_",s)] <- ifelse(sim_stats[data_iter,paste0("Fmsy_t_",s)] > Fmsy_e[,s][1] & 
                                                                          sim_stats[data_iter,paste0("Fmsy_t_",s)] < Fmsy_e[,s][3],
                                                                        "TRUE","FALSE")
      }
