...
stat_files <- str_remove(res_files[grep(paste0("^", prefix1), files_list)], "ko")
stat_files <- str_remove(stat_files[grep(paste0("^", prefix1), files_list)], "_Best")
stat_files <- str_remove(stat_files[grep(paste0("^", prefix1), files_list)], "_Best_Iest")
stat_files <- str_remove(stat_files[grep(paste0("^", prefix1), files_list)], "_Iest")
...
cols <- wes_palette("IsleofDogs1"); cols

## Divergent transitions: 
#I fucked up and only recorded divergent transitions for ko model :(
ggplot(master2 %>% filter(model == "ko")) +
  ...
  ylab("Bias in r") + xlab("Divergent transitions")
  ...
  ggplot(master2 %>% filter(model == "ko")) +
    ...
    geom_smooth(aes(prop_div, Kbias_1, col = harv_hist),
                method = 'loess', span = 1, se = FALSE) +
    geom_hline(yintercept = 0,linetype = "dashed", color = "red", size = 1) +
  ylab("Bias in K") + xlab("Divergent transitions")  
  ...
  ggplot(master2 %>% filter(model == "ko")) +
    ...
    geom_smooth(aes(prop_div, qbias_1, col = harv_hist),
                method = 'loess', span = 1, se = FALSE) +
    geom_hline(yintercept = 0,linetype = "dashed", color = "red", size = 1) +
    ylab("Bias in q") + xlab("Divergent transitions")  
