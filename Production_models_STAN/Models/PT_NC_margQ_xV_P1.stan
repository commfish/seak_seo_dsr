...
// PRIORS:  
  ...
  for (s in 1:S){
    ...
    mu[1,s] ~ normal(0,1); //normal(P1_mu,P1_sig); // T[,P1_mu * 0.6];
    for (t in 2:N){
      mu[t,s] ~ normal(0,1); //normal(P1_mu,1);
    }
  } 
...
for (s in 1:S){
  for (t in 2:N) {
    P[t,s] = fmax((P[t - 1,s] + (r / (pte - 1)) * P[t - 1,s] * (1 - pow(P[t - 1,s] , (pte - 1))) - C[t - 1,s] / (K * pi[s])) * exp(sigma*mu[t,s]), 0.001) * exp(PE[t-1]); 
