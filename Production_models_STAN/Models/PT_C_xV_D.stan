data {
  ...
  // CATCH DATA
  real<lower=0> C_obs[N,S]; // estimated treatment effects
  real C_cv[N,S]; // estimated treatment effects

  // DISCARD DATA
  real<lower=0> D_obs[N,S]; // estimated treatment effects
...
// Set initial state
  for (s in 1:S) {
    P[1,s] ~ lognormal(P1_mu,P1_sig) T[0.05,1.6]; 
    Pmed[1,s] = log(P[1,s]);
...
for (s in 1:S){
  ...
  for (t in 2:N) {
    Pmed[t,s] = log(fmax(P[t - 1,s] + (r / pte) * P[t - 1,s] * (1 - pow(P[t - 1,s] , pte)) - C_tot[t - 1,s] / (K * pi[s]), 0.001) * exp(PE[t-1])); //Pella-Tomlinson
    P[t,s] ~ lognormal(Pmed[t,s], sigma) T[0.05, 1.6]; //P[t,s] ~ lognormal(Pmed[t,s], sigma) T[0.05, 1.6];
