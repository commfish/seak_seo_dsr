data {
  ...
  real P1_mu;  //mean for prior on P1
...
// Set INITIAL state
  for (s in 1:S) {
    medP1[s] ~ lognormal(P1_mu,P1_sig) T[0.1,1.1];
    P[1,s] = medP1[s] * exp(sigma*mu[1,s]);
...
for (s in 1:S){
  ...
  for (t in 2:N) {
    P[t,s] = fmax((P[t - 1,s] + (r / (pte - 1)) * P[t - 1,s] * (1 - pow(P[t - 1,s] , (pte - 1))) - C_tot[t - 1,s] / (K * pi[s])) * exp(sigma*mu[t,s]), 0.001) * exp(PE[t-1]); //P[t,s] ~ lognormal(Pmed[t,s], sigma) T[0.05, 1.6];
