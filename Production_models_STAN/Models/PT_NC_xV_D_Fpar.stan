data {
  ...
  // DISCARD DATA
  real D_obs[N,S]; // estimated treatment effects
  real D_cv[N,S]; // estimated treatment effects
...
//  matrix<lower=0, upper=1>[N,S] F;
  matrix<lower=0, upper=1>[N,S] F;
  matrix<lower=0>[N,S] D;  
}
...
// PRIORS:  
  ...
  K ~ lognormal(k1, k2) T[10,50000];; //lognormal(10, 2) T[10,10000]; // uniform(100, 10000);
//  fsigma2 ~ gamma(0.001,0.001); 
  pi ~ dirichlet(alpha);
  for (s in 1:S){
...
// Set INITIAL state
  for (s in 1:S) { 
    ...
    F[1,s] ~ lognormal(-2.995732, fsigma); 
    logF[1,s] = log(F[1,s]);
    C[1,s] = F[1,s] * K * P[1,s] * pi[s];
    C_tot[1,s] = C[1,s] + D[1,s];
  }
...
for (s in 1:S){
  ...
  for (t in 2:N) {
    F[t,s] ~ lognormal(logF[t-1,s], fsigma); //lognormal(logF[t-1,s], fsigma);
    logF[t,s] = log(F[t,s]);
    C[t,s] = fmax(F[t,s] * K * P[t,s] * pi[s], 0.00001);
    C_tot[t,s] = C[t,s] + D[t,s];
    P[t,s] = fmax((P[t - 1,s] + (r / (pte - 1)) * P[t - 1,s] * (1 - pow(P[t - 1,s] , (pte - 1))) - C_tot[t - 1,s] / (K * pi[s])) * exp(sigma*mu[t,s]), 0.001) * exp(PE[t-1]); 
...
for (s in 1:S) {
  for (t in 1:N){
    ...
    logD[t,s] = log(D[t,s]);
    D_sig[t,s] = log(D_obs[t,s] + D_obs[t,s] * D_cv[t,s]) - log(D_obs[t,s]);   
    D_obs[t,s] ~ lognormal(logD[t,s],D_sig[t,s]);
...
if (ind1_switch == 0) { //if no index dont try to fit to the data
  ...
  for (i in 1:N_I1obs) {
    ...
    else if (i1_tau_vec[i] > 0) {
      usecvi1[i] = sqrt(I1_cv[i] * I1_cv[i] + tau_i1[i1_tau_vec[i]] * tau_i1[i1_tau_vec[i]]);
    } 
    I1_sig[i] = log(I1_obs[i] + I1_obs[i] * usecvi1[i]) - log(I1_obs[i]);
  }
  for (s in 1:S){
...
if (ind2_switch == 0) { //if no index dont try to fit to the data
  ...
  for (i in 1:N_I2obs) {
    ...
    else if (i2_tau_vec[i] > 0) {
      usecvi2[i] = sqrt(I2_cv[i] * I2_cv[i] + tau_i2[i2_tau_vec[i]] * tau_i2[i2_tau_vec[i]]);
    } 
...
//  real EMSY;
  ...
  for (s in 1:S) {
    P[1,s] = exp(sigma*mu[1,s]);
    for (t in 1:N) {
      C[t,s] = F[t,s] * K * P[t,s] * pi[s];
    //  logF[t,s] = log(F[t,s]); 
      C_tot[t,s] = C[t,s] + D[t,s];
    }
    for (t in 2:N) {
      P[t,s] = fmax((P[t - 1,s] + (r / (pte - 1)) * P[t - 1,s] * (1 - pow(P[t - 1,s] , (pte - 1))) - (C[t - 1,s] + D[t - 1,s]) / (K * pi[s])) * exp(sigma*mu[t,s]), 0.001);
    }
    for (t in 1:N) {
      Biomass[t,s] = K * P[t,s] * pi[s];
      log_bio[t,s] = log(Biomass[t,s]);
  ...
  if (ind2_switch == 0) {
    ...
    for (i in 1:N_I2obs) {
      ...
      I2_sig[i] = log(I2_obs[i] + I2_obs[i] * usecvi2[i]) - log(I2_obs[i]);
